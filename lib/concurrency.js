...
let is_fd_locked = false

const write_range_data = (response, urldata, resolve, reject) => {
  ...
  range_data.push({response, file_descriptor, first_byte, range_size, resolve, reject})

  write_to_fd()
}

const write_to_fd = () => {
  if (is_fd_locked || !range_data.length)
    return

  is_fd_locked = true

  let data = range_data.shift()

  const {file_descriptor, first_byte, range_size, resolve, reject} = data
  ...
  fs.write(file_descriptor, data.response, 0, range_size, first_byte, (error, bytesWritten) => {
    ...
    else if (bytesWritten !== range_size) {
      const err = 'Bytes written to output file for Range is incorrect length'
...
module.exports = {get_url_ranges, write_range_data}
