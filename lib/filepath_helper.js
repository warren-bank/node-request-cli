...
const get_output_filepath = (argv_vals, urldata, {url, redirects, response, is_html} = {}) => {
  ...
  if (!output_filepath && argv_vals["--recursive"] && request_url) {
    ...
    }
    {
      ...
      fname = normalize_url_basename(argv_vals, parts.pop()) || argv_vals["--default-page"] || 'index.html'
      ...
      if (parsed_url.query && !argv_vals["--no-querystring"]) {
        fname += (argv_vals["--restrict-file-names"].indexOf("windows") >= 0)
          ? '@'
          : '?'
        fname += normalize_url_querystring(argv_vals, parsed_url.query)
      }
    }
  ...
  if (!output_filepath && request_url && (!argv_vals["--content-disposition"] || (argv_vals["--content-disposition"] && (response instanceof Object)))) {
    ...
    let   fname      = normalize_url_basename(argv_vals, parts.pop()) || argv_vals["--default-page"] || 'index.html'
...
const normalize_url_basename = (argv_vals, url_basename) => {
  return normalize_url_component(argv_vals, url_basename, "uri-encode-basename")
}

const normalize_url_querystring = (argv_vals, url_querystring) => {
  if (url_querystring) {
    // decode special characters
    url_querystring = url_querystring.replaceAll('+', ' ')
  }

  return normalize_url_component(argv_vals, url_querystring, "uri-encode-querystring")
}

const normalize_url_component = (argv_vals, url_component, option_value) => {
  if (!url_component) return null

  // 1st pass: remove URI encoding (if present)
  url_component = decodeURIComponent(url_component)

  // 2nd pass: apply URI encoding (optional)
  if (argv_vals["--restrict-file-names"].indexOf(option_value) >= 0) {
    url_component = encodeURIComponent(url_component)
  }
