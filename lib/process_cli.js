...
const process_cli = function(argv_vals){
  ...
  const download_next_url = function(){
      ...
      // short-circuit HTTP request when 'no-clobber' and the output filename is not determined by an HTTP response header
      if (argv_vals["--no-clobber"]) {
        let output_filepath
        if (Array.isArray(urldata) && (urldata.length >= 2)) {
          output_filepath = urldata[1]
        }
        else if (argv_vals["--output-document"] && (!Array.isArray(urldata) || (argv_vals["--output-document"] === "-"))) {
      ...
      .then(({url, redirects, response}) => {
        ...
        return new Promise((resolve, reject) => {
          const stream = (output_filepath !== "-")
            ? fs.createWriteStream(output_filepath)
            : process.stdout
