...
const html_exts = ['', '.asp', '.apsx', '.cfm', '.cgi', '.dhtml', '.hta', '.htm', '.html', '.jhtml', '.jsp', '.php', '.php2', '.php3', '.php4', '.php5', '.php6', '.php7', '.php8', '.php9', '.phps', '.pht', '.phtm', '.phtml', '.pl', '.shtml']
...
const apply_crawler_middleware = async (argv_vals, urls, {url, redirects, response, urldata}) => {
  ...
  let current_depth, was_html, is_html, is_css
  ...
  if (should_crawl) {
    ...
    was_html = (Array.isArray(urldata) && (urldata.length >= 4))
      ? urldata[3]
      : undefined
  ...
  if (should_crawl) {
    ...
    if (typeof was_html === 'boolean') {
      // IMPORTANT: do not change output filepath! Previously crawled files are already linked.

      // Display a notification when the earlier determination of whether the URL contains HTML content, based only on its path, was incorrect:
      if (was_html !== !!is_html) {
        log(`WARNING: The following URL ${was_html ? 'was' : 'was not'} determined to contain HTML content by inspection of its pathname:`)
        log('  ', url)

        if (
          (!was_html && is_html.mime) ||
          ( was_html && response.headers && response.headers['content-type'])
        ) {
          log('However the "Content-Type" of the server response says otherwise:')
          log('  ', response.headers['content-type'])
        }
      }
    }
    else if (is_html && is_html.force) {
...
const process_regex_url_rule = (text, regex_url_rule, replacer) => {
  ...
  return text.replace(_regex, (...matches) => {
    ...
    if ((matches.length > match_index) && matches[match_index]) {
      ...
      let is_html = false
      if (regex_url_rule.length > 2) {
        const _html = regex_url_rule[2]

        is_html = (typeof _html === 'function')
          ? _html(matches)
          : !!_html
      }

      const new_url = replacer(old_url, is_html)
...
const process_new_url = (argv_vals, new_urls, {url, redirects, current_depth, follow_html}, new_url, is_html) => {
  if (!new_url) return ''
  ...
  if (!is_html) {
    is_html = should_force_html(argv_vals, new_url)
  }

  const follow_url = (is_html && !follow_html)
    ? false
    : should_follow_url(argv_vals, new_url, is_html)
...
const should_follow_url = (argv_vals, new_url, is_html) => {
  ...
  if (samehost && argv_vals["--no-parent"] && (pathname.indexOf(root.pathname) !== 0) && (is_html || !argv_vals["--page-requisites"])) {
    return false
  }
